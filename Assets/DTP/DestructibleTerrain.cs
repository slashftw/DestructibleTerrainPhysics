...
namespace slash_DTP
{
    ...
    public class DestructibleTerrain : MonoBehaviour
    {
        ...
        public class TextureSettings
        {
            ...
            [Tooltip("Automatically fix incompatible texture import settings at runtime (Editor only). When enabled, textures with incorrect settings (Read/Write disabled, wrong sprite mode, etc.) will be permanently reimported with correct settings on Play.")]
            public bool autoFixImportSettings = true;
        }
        ...
        // once with arrayLength=0 forces the compile during scene Init (~60ms one-time cost
        ...
        // 6F removes the managed copies once everything reads from native.
        ...
        // per logical chunk. Each pc.SetPath rebuilds a Box2D fixture (~5-10ms),
        // so 100 paths = 500ms+ per chunk rebuild. Profile showed RebuildChunk
        // Max 758ms in this scenario. These tiny pieces will either become real
        ...
        // by area kept; rest dropped. 16 paths × ~5ms pc.SetPath = ~80ms worst
        // case rebuild — still high but bounded vs the 758ms unbounded case.
        ...
        // ApplyParallelRebuildResult sub-sections (the 8.76ms spike target).
        ...
        // ExtractAndSpawnIsland sub-sections (the 14.21ms spike target).
        ...
        // StampAndDestroy sub-sections (the 9.35ms merge spike target).
        ...
        // StampAndDestroy is ~30ms (Texture2D destroy + visual sub-chunk destroy
        ...
        // a 150ms+ spike occurs. Capping to MAX_MERGES_PER_FRAME spreads them.
        // The user perceives a slightly delayed merge animation (~50-150ms), much
        // better than a single big stutter.
        // Bumped from 1 to 2 because users perceived the 1-merge-per-frame cap as
        // visible delay when many small chunks settled at once. 2/frame is a
        // compromise — peak ~60ms, still allows 6 merges over 3 frames vs 6 in 1.
        ...
        // replaces the managed List<List<Vector2>> output with Burst-friendly
        ...
        // + traceWorldOffset replaces the padded → unpadded → step → invPpu chain.
        ...
        // Replaces the synchronous spawn-site RebuildChunk loop that was costing
        // 70-100ms per spawn frame. New falling chunks spawn with rb.simulated=false
        ...
        // shared 2MB list is safe and saves ~620MB across 310 component instances
        ...
        // by OnCollisionStay2D's mergeTarget search; saves up to ~65ms in pile
        ...
        private const float REST_ANGULAR_THRESHOLD = 30f;
        ...
        // --- HEAVY OPT: Target physics resolution for falling debris ---
        ...
        // ENTIRE chunk's collider, causing a 47ms pc.SetPath spike per dig on a
        ...
        // (~1-2ms). Per-dig touches 1-4 chunks = ~3-8ms.
        // Briefly tried 256 (16×16 grid) for finer per-rebuild cost. Profile
        // confirmed RebuildChunk was already mainland-equivalent at 512 — the
        // residual lag was the BFS Complete stall (fixed by Phase 14 non-blocking
        // BFS). Reverted to 512 to keep Box2D fixture count manageable on mobile
        // (256 = 4× more dynamic fixtures per huge chunk).
        private const int FALLING_CHUNK_PIXEL_DIM = 512;

        // Falling chunks above this dimension switch to visual sub-chunks (Phase 7).
        ...
        // of the full huge texture (was costing 30+ms per dig frame on mobile for
        ...
        // via Texture2D.Reinitialize, avoiding the 10-30ms cost of `new Texture2D`
        ...
        // + CreateChunks + dirty-flagging — ~5–15ms) is staged into the dt's next
        ...
        // --- THE GC & CRASH FIX: Everything must be static! ---
        ...
        // --- TIER 1: FAST CHUNK SIGNATURES ---
        private int[] chunkSignatures;

        // per-chunk per-path signature hash. SetPath is the dominant
        // cost in RebuildChunk now (14ms for 5-10 paths). Most rebuilds change
        ...
        // Per-spawn savings: ~3ms per visual chunk × up to 16 sub-chunks = ~48ms.
        ...
        // was `=> visualChunks != null`, but Phase 7 added visual
        ...
        public struct TerrainConfig
        {
            ...
            public int groundAnchorY, maxEmbedPixels, aftermathThickness;
        ...
        void InitMarchingSquaresBuffers(int effectiveCells)
        {
            ...
            // 6A-6E moved every consumer to NativeArrays. The old managed mirrors
            // (padBuffer, nextNode, vertexVisited, usedNodes) are no longer
            // allocated — saves ~630KB of dead managed memory at startup.
            ...
            // 6D: contour output containers. Capacities sized to typical worst case
            ...
            // the first heavy gameplay frame. Eliminates the ~60ms first-use stutter
            // observed in editor profile (DT.Extract.AllocSrc max 67.98ms is a chained
            // Burst compile on first dig event).
        ...
        // Profile data (deep-profile, dev build, big-falling-chunk merge):
        //   StampAndDestroy: 176 ms total, dominated by 400k NativeArray.set_Item
        //                    safety checks (CheckElementWriteAccess + AtomicSafetyHandle).
        //   ComputeChunkSignature: 64 ms total, 131k NativeArray.get_Item safety checks.
        ...
        // Burst-compiled merge stamp. Replaces the old #if UNITY_EDITOR managed loop
        // and the #else native loop — same logic, no safety overhead, ~10-20x faster
        // in the editor and ~2-3x faster in release.
        [BurstCompile]
        ...
        // Profile data (Phase 8 baseline, dev build):
        //   ProcessEraseCircle: 8.19ms / 4 ErasePixelsInChunkGrid calls
        //                       16,322 NativeArray accesses (mostly safety checks).
        //   solidMap[index] reads + writes dominate; managedRawPixels is fast.
        ...
        // costs ~50-100us of dispatch overhead. With 16 dirty chunks × 5 jobs each
        // we'd pay 4-8ms in dispatch alone. One unified job amortizes dispatch over
        // the entire batch.
        ...
        // DetectFloatingIsland) was costing 5-50ms/frame on the main thread during
        ...
        // gets a 1-frame latency (8.3ms at 120fps) — imperceptible.
        ...
        // Phase 5 per-instance fields. Only mainland actively uses them — falling
        ...
        private static void ResetStaticNativeArrays()
        {
            ...
            // 6E: simplified contour outputs and RDP scratch.
...
#if UNITY_EDITOR
        private bool _textureFixInProgress;
#endif
        ...
        void OnValidate()
        {
            ...
            if (spriteRend == null || spriteRend.sprite == null) return;
            
            Texture2D texture = spriteRend.sprite.texture;
            if (texture == null) return;
            
            if (!texture.isReadable && textureSettings.autoFixImportSettings)
            {
                UnityEditor.EditorApplication.delayCall += () =>
                {
                    if (this == null || spriteRend == null) return;
                    AutoFixTextureImportSettings();
                };
            }
        }

        private void AutoFixTextureImportSettings()
        {
            if (spriteRend == null || spriteRend.sprite == null) return;
            ...
            _textureFixInProgress = true;
            Debug.LogWarning($"[DestructibleTerrain] Auto-fixing texture '{texture.name}' import settings...", this);
            ...
            if (importer != null)
            {
                ...
                importer.maxTextureSize = Mathf.Max(texture.width, texture.height, textureSettings.maxResolution);
                ...
                UnityEditor.EditorApplication.delayCall += () =>
                {
                    ...
                    foreach (var asset in assets)
                    {
                        if (asset is Sprite sprite)
                        {
                            ...
                            _textureFixInProgress = false;
                            Debug.Log($"[DestructibleTerrain] Successfully fixed sprite '{texture.name}'. Press Play again to use it.", this);
                            
                            if (Application.isPlaying && _awakeDeferred)
                            {
        ...
        {
#if UNITY_EDITOR
            // If texture fix is in progress, defer Awake until it completes
            if (_textureFixInProgress)
            {
                _awakeDeferred = true;
                return;
            }
            ...
            config = new TerrainConfig
            {
                ...
                groundAnchorY = this.groundAnchorY,
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                // Eager 5M-byte alloc per pool object × 300 was costing ~5s at startup.

                // Phase 25 ROLLED BACK: tried pre-allocating _pooledTexture at
                // FALLING_CHUNK_PIXEL_DIM and pre-creating _vcPool entries at
                // visualChunkSize. Premise was that Texture2D.Reinitialize is
                // cheap. It is not — Reinitialize on a size change does
                // free-then-alloc internally (Unity docs explicitly state this).
                // Forcing every spawn through Reinitialize-to-smaller-size made
                // ExtractAndSpawnIsland Mean regress 7.01ms → 16.91ms, MergeChunk
                // Median 0.92ms → 6.86ms. CreateVisualChunks did improve
                // (19.52ms → 5.76ms median) because typical 256×256 visual chunks
                // matched the prewarmed size and hit the Reinitialize no-op
                // fast-path, but the loss elsewhere outweighed it.
                // The natural Phase 16 pool (stash on StampAndDestroy, reuse on
                // ExtractAndSpawnIsland) is the right design — when same-size
                // texture is reused, Reinitialize is a no-op. Forcing different
                // sizes to converge on a fixed prealloc was the mistake.
        ...
        {
            this.config = newConfig; // Save the suitcase
            ...
            this.groundAnchorY = newConfig.groundAnchorY;
        ...
        /// <summary>
        /// FIX 4: Single-chunk + downsampled physics for falling debris.
        /// Replaces the per-grid-cell rebuild approach, dropping RebuildChunk calls from 256 to 1.
        /// </summary>
        ...
        private void InitializeAsFallingChunkSingle(NativeArray<int> activeLocalPixels, int pixelCount, int spriteW, int spriteH)
        {
            ...
            // rebuilt that one giant contour at ~47ms pc.SetPath cost. Now
            ...
            // copy + tex.Apply at creation (was costing 99ms max per spawn).
            ...
            // Phase 26 (Option α): falling chunks ALWAYS use the parent
            // SpriteRenderer's single Texture2D — never visual sub-chunks. The
            // previous size-gate (>512px → sub-chunks) is removed because deep
            // profile showed CreateVisualChunks costs 5-35ms (Median 19.52ms) per
            // big-island spawn, which dwarfed the per-frame Apply savings sub-
            // chunks were meant to provide. Single full tex.Apply on a 1024²
            // texture is ~3ms — much cheaper than the sub-chunk creation overhead.
            // Mainland still uses sub-chunks (CreateVisualChunks called once at
            // scene Init). Falling chunks rely on Phase 16's pooled fallingTex and
            // ApplyVisuals's visualChunks==null branch (`tex.Apply(false)`).
            // FALLING_VISUAL_CHUNK_MIN_DIM constant is no longer referenced; left
            // in place as a tombstone. _deferredParentDisable is permanently false.
            ...
            // would cost ~100-130ms at spawn. Instead, mark all chunks dirty and
        ...
        private void SetupAsBoxDebris(NativeArray<int> activeLocalPixels, int pixelCount, int spriteW, int spriteH)
        {
            ...
            // --- STRIKE 2: BULLETPROOF MICRO-DEBRIS ---
            int totalPixels = texWidth * texHeight;
            if (!solidMap.IsCreated || solidMap.Length < totalPixels)
            {
                if (solidMap.IsCreated) solidMap.Dispose();
        ...
        void LateUpdate()
        {
            ...
            if (_hasPendingDetection)
            {
                ...
                } // end of Phase 14 IsCompleted else
            }

            // --- THE PIPELINE CHOKE FIX ---
            ...
            // --- GC TABLE FIX: DEFENSIVE FOR LOOPS ---
            ...
            // --- OPTIMIZATION 3.7 & TASK 2: LOCALIZED BRESENHAM SCANNING ---
            if (enableIslandFalling)
            {
                ...
                // Detection latency: 1 frame (~8ms at 120fps). Imperceptible.
                ...
                if (pendingScans.Count > 0 && Time.frameCount - _lastDigFrame >= DETECT_DEBOUNCE_FRAMES && !_hasPendingDetection && !_globalBfsInFlight)
                {
                    ...
                    // in flight at a time. Replaces Phase 13's blunt every-15-frames
                    // throttle. Now: BFS reschedules immediately when the previous
            ...
            // 10-30× depending on how many polygon paths actually changed (Phase 20
            // hash-skip eliminates unchanged ones, but paths near the brush genuinely
            ...
            // 24-30ms of work — the spike profile shows 34 SetPath calls = 26ms
            // DestroyShapes. The time budget clips that to ~rebuildBudgetMs without
            ...
            // per frame, ~3-4× wall-clock speedup on the marching-squares portion
            // and elimination of ~0.5ms × 5 = 2.5ms of per-rebuild dispatch overhead.
            ...
            // MOVED TO FixedUpdate() - was causing 1-frame delay where chunks had no collision
            // See FixedUpdate() below
        }

        // ADDED: Process physics activations in FixedUpdate so colliders are active BEFORE physics runs
        void FixedUpdate()
        {
        ...
        void OnCollisionEnter2D(Collision2D collision)
        {
            ...
            if (this.rb != null && this.rb.bodyType != RigidbodyType2D.Kinematic)
            {
                ...
                // IsMainland (Phase 7: backed by explicit flag) instead of
                // `visualChunks != null` because Phase 7 added visual sub-chunks
        ...
        void OnCollisionStay2D(Collision2D collision)
        {
            ...
            if (mergeTarget == null)
            {
                ...
                if (sleepIdleSeconds > 0f && _restTimer >= sleepIdleSeconds && rb != null && rb.bodyType == RigidbodyType2D.Dynamic)
                {
                    ...
                    // CHANGED: Use Kinematic instead of Static to preserve collision with Dynamic bodies
                    // Static bodies don't collide with Dynamic, causing falling chunks to phase through
                    rb.bodyType = RigidbodyType2D.Kinematic;
        ...
        DestructibleTerrain FindNearbyMainland()
        {
            ...
            // Physics2D.OverlapBoxNonAlloc — profile showed Max 65.61ms when many
        ...
        private void StampAndDestroy(DestructibleTerrain fallingChunk)
        {
            ...
            // Phase 5 safety: StampAndDestroy is invoked from the CrunchAndMerge
            ...
            // used to run unconditionally here. With Phase 14b's NativeDisable on
            ...
            // costing 50-100ms per merge during sustained dig (BFS continuously
            // in flight), making each large-chunk merge a 130ms LateUpdate spike.
            ...
            // --- INNER-LOOP FLOAT HOIST ---
            // The Vector3 += stepX in the per-pixel loop was eating ~120ms of profile
            // time on big stamps because Vector3.op_Addition wasn't being inlined.
            ...
            // BURST MERGE STAMP
            // Replaces the old #if UNITY_EDITOR managed loop and the #else native loop.
            // Profile showed 176ms in the merge dominated by 400k NativeArray safety
            // checks. Burst eliminates that entirely. Particle emission is buffered
            ...
            // Each ParticleSystem.Emit is ~0.05ms but in aggregate (1000+) becomes
            // a 50-100ms spike. Cap to MAX_PARTICLES_PER_MERGE to keep merge frames
            ...
            if (mapChanged)
            {
                ...
                if (fallingSr.sprite != null)
                {
                    ...
                    // view; ~1ms recreation cost). Saves 10-30ms per spawn cycle
                    // by skipping `new Texture2D` allocation.
        ...
        private bool CloneTexture()
        {
            ...
            {
#if UNITY_EDITOR
                // During Play mode: check if fix is in progress from Edit mode OnValidate
                // If so, defer; if not and auto-fix enabled, stop Play and show error
                if (Application.isPlaying)
                {
                    if (_textureFixInProgress)
                    {
                        // Fix already in progress from Edit mode, just defer
                        _awakeDeferred = true;
                        return false;
                    }
                    else
                    {
                        // Texture not fixed before Play - this shouldn't happen with OnValidate
                        // Trigger fix now but warn user it won't persist
                        if (textureSettings.autoFixImportSettings)
                        {
                            Debug.LogWarning($"[DestructibleTerrain] Texture '{texture.name}' being fixed during Play mode. Sprite may unassign after exit. Re-enter Play mode to persist.", this);
                            AutoFixTextureImportSettings();
                            _awakeDeferred = true;
                            return false;
                        }
                ...
                {
                    // In Edit mode - trigger fix immediately
                    if (textureSettings.autoFixImportSettings)
                    {
                        AutoFixTextureImportSettings();
                        return false;
                ...
                Debug.LogError(
                    ...
                    "Enable 'Auto Fix Import Settings' in Texture Settings.", this);
                enabled = false;
                return false;
            }
        ...
        private void ProcessEraseCircle(Vector2 worldPos, float radius)
        {
            ...
            // --- THE RIM FIX: DELETE THE BOUNDS CHECK ---
            ...
            // BURST DIG
            // Replaces the per-chunk ErasePixelsInChunkGrid loop. Profile showed 8ms
            // dominated by NativeArray safety on ~16k solidMap reads/writes; Burst
            // strips that overhead. Operates on the whole brush AABB in one job.
        ...
        // Profile data: Phase 7's CreateVisualChunks was costing 99ms max per spawn
        // (16-256 sub-chunks each doing a per-row pixel copy + tex.Apply). For a
        // 4096² spawn this is ~250ms of stalled main thread.
            ...
            for (int vy = 0; vy < visualChunksY; vy++)
            {
                for (int vx = 0; vx < visualChunksX; vx++)
                {
                    ...
                    else
                    {
                        ...
                        // costs 1.74ms / 2.1KB GC across 42 cold-fill chunks.
                    ...
                    // sprite across reuses — saves ~0.5ms per chunk per spawn.
        ...
        void DestroyFallingVisualChunks()
        {
            ...
            // SpriteRenderer + Texture2D. Saves ~3ms per visual chunk × up to 16
            // chunks = ~48ms per spawn. Sprite stays attached for potential reuse
        ...
        void ApplyVisuals()
        {
            if (visualChunks != null)
            {
                ...
                // 45+ visual chunks become fully dirty in one frame (was costing 32ms
                ...
                // normal but still bounded — unlimited drain on a big merge was costing
                // 50+ms, defeating the whole throttle.
                int budget = forceDrainVisuals ? Mathf.Max(maxVisualsPerFrame, 32) : maxVisualsPerFrame;
                forceDrainVisuals = false;
                // time-budget mirror of Phase 21's RebuildChunk loop.
                // ApplyVisuals Max was 19.74ms — same shape as the RebuildChunk
            ...
            else
            {
                ...
                // ApplyVisuals consumes + clears it. Saves ~0.5–1ms per active
                // falling chunk per frame in editor; smaller win on mobile but
                // still meaningful at 5+ active chunks.
        ...
        GameObject ExtractAndSpawnIsland()
        {
            ...
            // reuse pooled Texture2D via Reinitialize. Skips the ~10-30ms
            ...
            // Was a dual editor/release path. Editor's managed loop iterated up to 5M
            // island pixels reading/writing managedRawPixels (~100ms for huge spawns).
            // Release path used Burst job with NativeArrays.
            // Phase 11 collapsed to a single Burst path running ExtractTransformReleaseJob
            // on rawPixels directly. Phase 34 then removed the editor managedRawPixels
            // mirror entirely, so rawPixels is now canonical in both editor and release.
            ...
            // extraction boundary. Costs ~1-5ms per spawn (iterates the island
            ...
            // dt was acquired earlier (above Phase 16 texture alloc).

            // 1. Hand the suitcase to the falling chunk so it unpacks everything automatically!
            dt.ApplyConfig(this.config);

            // 2. Override ONLY the 2 variables that MUST be different for falling debris
            dt.groundAnchorY = -1;  // Falling chunks should not anchor to ground, might need to set it as null ...
            dt.isMerging = false;

            // Now that Awake() is safely out of the way, we can manually initialize the chunk!
            // defer the heavy Init work (~5–15ms: ClearByteArrayJob +
        ...
        void CreateChunks()
        {
            ...
            { // <-- Added bounds check!
                ...
                chunkColliders = new PolygonCollider2D[newSize]; // Phase 22
                isChunkDirty = new bool[newSize];
                chunkSignatures = new int[newSize]; // <-- The missing allocation!
            ...
            for (int cy = 0; cy < chunksY; cy++)
            {
                for (int cx = 0; cx < chunksX; cx++)
                {
                    ...
                    chunkObjects[idx] = go;
                    chunkColliders[idx] = pc; // Phase 22: cache for RebuildChunk.
                }
            }
        ...
        void PopulateIsolatedPadBuffer(int cx, int cy, int step)
        {
            ...
            // padBufferNative directly. Managed padBuffer was eliminated in 6F.
        ...
        void ApplyParallelRebuildResult(int slot, int idx)
        {
            ...
            // costs ~3ms via Box2D fixture array resize).
        ...
        // Notice the default '= 1' is removed here!
        bool RebuildChunk(int cx, int cy, int step)
        {
            int idx = cx + cy * chunksX;

            // --- FAST CHUNK SIGNATURE PRE-CHECK ---
            ...
            var pc = chunkColliders[idx]; // Phase 22: cached, no GetComponent.

            // filter tiny contour paths. With heavy fragmentation a chunk
            // can produce 100+ contours, and pc.SetPath is ~5-10ms per Box2D fixture
            ...
            // pc.pathCount setter is ~3ms per change (Box2D fixture
            ...
            // path (the one near the dig). pc.SetPath costs ~1-3ms per call and
        ...
        // Notice the updated int[] signature
        ...
        { // grid param retained for caller compat
            ...
            // reads the managed list (will migrate in 6D). Bulk index copy is
            // ~50us at worst-case 16k edges, irrelevant compared to the savings.
            new BuildEdgesJob
            {
                padBuffer = padBufferNative,
                caseLineCount = caseLineCount,
            ...
            // 6D: TraceContours now reads edgesNative directly — managed `edges`
            // list is no longer needed. Sync removed; will clean up the field
            // declaration in 6F.
        }
        void TraceContours()
        {
            ...
            // into contoursVertsNative + contourSpansNative (CSR layout). 6E reads
            // these directly via RDPSimplifyJob — no managed hydration needed.
