...
namespace slash_DTP
{
    ...
    {
        const string DemoSpriteName = "4096x4096_white";
        ...
        {
            // Find demo sprite anywhere in Assets/DTP folder
            string[] guids = AssetDatabase.FindAssets($"{DemoSpriteName} t:Texture2D", new[] { "Assets/DTP" });
            
            if (guids.Length == 0)
            {
                Debug.LogWarning($"[DestructibleTerrain] Demo sprite '{DemoSpriteName}' not found in Assets/DTP. " +
                    "Created terrain has no sprite — assign one (readable, uncompressed RGBA32, no mip maps) to its SpriteRenderer.");
                return null;
            }
            ...
            {
                Debug.LogWarning($"[DestructibleTerrain] Demo sprite at '{spritePath}' is not a texture. Skipping.");
                return null;
            }
